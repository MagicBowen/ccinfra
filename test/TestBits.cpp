...
TEST(BitsTest, should_calc_the_correct_bit_mask_by_macro)
{
...
TEST(BitsTest, should_calc_the_correct_bit_mask_by_template_function)
{
...
TEST(BitsTest, should_get_the_bits_value_by_macro)
{
...
TEST(BitsTest, should_get_the_bits_value_by_template_function)
{
...
TEST(BitsTest, should_get_the_given_bit_value_by_macro)
{
    ASSERT_FALSE(IS_BIT_ON(0xaa, 2));
    ASSERT_TRUE(IS_BIT_ON(0xccaaaaaaaaaaaaaa, 63));
}

TEST(BitsTest, should_get_the_given_bit_value_by_template_function)
{
    ASSERT_FALSE(is_bit_on(0xaa, 2));
    ASSERT_TRUE(is_bit_on(0xccaaaaaaaaaaaaaa, 63));
}
