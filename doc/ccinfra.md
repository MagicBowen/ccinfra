...
- 通过多重继承的方式，事实上可以同时完成类的组合以及依赖注入，本质上是一种组合式编程。被继承在同一颗继承树上的类天然被组合在一起，同时通过`USE_ROLE`和`IMPL_ROLE`的这种编织虚函数表的方式完成了这些类之间的互相依赖引用，相当于完成了依赖注入，只不过这种依赖注入成本更低，表现在C++上来说就是避免了每个类中去定义依赖注入的指针以及通过构造函数进行注入操作，而且同一个领域对象类的多个对象共享类的虚表，所以更加节省内存。

- 这种多重继承类似建立了一颗倒置的树。领域对象只是完成role的组合和依赖关系交织，当需要使用某一个role时，必须从领域对象cast到对应的role上，不会出现传统教科书上所说的多重继承带来的二义性问题。

- 采用这种多重继承的方式组织代码，我们会得到一种小类大对象的结构。所谓小类，指的是每个role的代码是为了完成组合和扩展性，是站在类的角度去解决工程性问题，一般都相对较小。而当不同的role组合到一起形成大领域对象后，它却可以让我们站在领域的角度去思考问题，关注领域对象整体的领域概念关系和生命周期而不用关心那些琐碎的role。
